 ...
 * 
 * <pre>
 *   java KImageInfo *.jpg *.png *.gif http://somesite.tld/image.jpg
 * </pre>
 * 
 * or call it without parameters and pipe data to it:
 * 
 * <pre>
 *   java KImageInfo &lt; image.jpg
 * </pre>
 * <p/>
 * Known limitations:
 * <ul>
 ...
 * returns whether KImageInfo has found that the storage type is progressive (or
...
public class KImageInfo {
	...
	private static Logger logger = Logger.getLogger(KImageInfo.class);

	private static HashMap<String, KImageInfo> imageMap = new HashMap<String, KImageInfo>();

	@SuppressWarnings("unused")
	private class MySSLVerifier implements HostnameVerifier {
	...
	public static KImageInfo getInstance(String name) throws IOException {
		KImageInfo info = imageMap.get(name);

		if (info == null) {
			info = new KImageInfo(name);
			imageMap.put(name, info);
		}
	...
	/**
	 * Return value of {@link #getFormat()} for JPEG streams. KImageInfo can
	...
	/**
	 * Return value of {@link #getFormat()} for GIF streams. KImageInfo can
	 ...
	 * with PNG, but KImageInfo is currently not able to extract those. It is
	...
	public static final String[] MIME_TYPE_STRINGS = { "image/jpeg",
			"image/gif", "image/png", "image/bmp", "image/pcx", "image/tiff",
	...
	public KImageInfo(URL url) throws IOException {
		InputStream in = null;
	...
	public KImageInfo(String name) throws IOException {
		InputStream in = null;

		if (name.startsWith("http://") || name.startsWith("https://"))
			in = new URL(name).openConnection().getInputStream();
		else {
			// see if it's an object located in the classpath
	...
	public KImageInfo(File imageFile) throws IOException {
		this(new FileInputStream(imageFile));
	}

	public KImageInfo(InputStream inputStream) throws IOException {
		setInput(inputStream);
		if (!check())
			throw new IOException("Unknown image type");
	}

	public KImageInfo(DataInput dataInput) throws IOException {
		setInput(dataInput);
		if (!check())
			throw new IOException("Unknown image type");
	}

	public KImageInfo(byte[] data) throws IOException {
		/*
		 * int length = data.length;
		 * 
		 * PipedOutputStream pos = new PipedOutputStream(); pos.write(data, 0,
		 * length);
		 * 
	...
	private InputStream getInputStream(String objectName) {
		...
		in = cl.getResourceAsStream(objectName);
	...
	public static void main(String[] args) {
		KImageInfo imageInfo = new KImageInfo();
	...
	private static void print(String sourceName, KImageInfo ii, boolean verbose) {
		if (verbose) {
	...
	private static void printCompact(String sourceName, KImageInfo imageInfo) {
	...
	private static void printVerbose(String sourceName, KImageInfo ii) {
	...
	protected void synchBits() {
		bitBuf = 0;
		bitPos = 0;
	}

	protected String readLine(int firstChar) throws IOException {
	...
	private static void run(String sourceName, InputStream in,
			KImageInfo imageInfo, boolean verbose) {
